  ...
  return (
    <div className="gameInfo">
      ...
      <div className="gameuser">
        <p>
          모집 인원 {aboutGame.gameInfo.minPeople}~{aboutGame.gameInfo.maxPeople}명
        </p>
        <p>
          현재 모인 인원은 <span>{aboutGame.gameParticipantList.length}</span> 명 입니다.
        </p>
        <p>
          참가비 <span>{aboutGame.gameInfo.participationFee}</span> 원
        </p>
      </div>
      ...
      <div className="participants">
        {users.map((user, idx) => {
          return <Participant key={idx} id={user.id} user={user} />;
        })}
      </div>
    </div>
  );
