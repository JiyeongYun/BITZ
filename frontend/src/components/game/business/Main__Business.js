...
function Main__Business({ history }) {
  ...
  useEffect(() => {
    ...
    GameApi.requsetBusinessGameList(
      data,
      (res) => {
  ...
  gameLists.sort((a, b) => {
    const result =
      Number(String(a.startTime).slice(0, 2)) - Number(String(b.startTime).slice(0, 2));
    if (result < 0) {
      return -1;
    } else {
      return 1;
                    ...
                    return (
                      <div key={game.id} className="game_container">
                        <div className="game__time">
                          {String(game.startTime).slice(0, 5)} ~ {String(game.endTime).slice(0, 5)}
                        </div>
                        <div className="gym__info">
                          <h5>{game.gym.name}</h5>
                          <p>
                            {game.gym.sido} {game.gym.gugun}
                          </p>
                          <p>
                            {game.gym.courtLength}m X {game.gym.courtWidth}m
                          </p>
                        </div>
                        ...
                        <div className="participant">
                          <p>
                            모집 인원 : <span>{game.participant}</span>/{game.maxPeople}
                          </p>
                          <p>
                            입금 확인 요청 : <span>{game.participant}</span>명
                          </p>
                        </div>
                        <Link
                          to={{ pathname: '/detail/' + game.id, state: { isBusiness: true } }}
                          className="game__btn"
                        >
                          모집중
                        </Link>
